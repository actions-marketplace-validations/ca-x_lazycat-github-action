...
run_download_case() {
  ...
  mkdir -p "${action_tmp}"
  : >"${tmp}/curl.log"
  output="$(TMPDIR="${action_tmp}" PATH="${fake_bin}:${PATH}" FIXTURE_DIR="${fixtures}" CURL_LOG="${tmp}/curl.log" GITHUB_ACTION_REPOSITORY=wcaqrl/lazycat-github-action RUNNER_OS=Linux RUNNER_ARCH="${runner_arch}" LAZYCAT_ACTION_VERSION=v1.0.0 bash "${root}/scripts/run-action.sh")"
  grep -q "https://github.com/wcaqrl/lazycat-github-action/releases/download/v1.0.0/lazycat-action_linux_${archive_arch}.tar.gz" "${tmp}/curl.log"
